...
class BarbeiroViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        media_de_avaliacao = self.request.query_params.get(
            'media_de_avaliacao'
        )
        ...
        if por_barbearia:
            queryset = queryset.filter(
                barbearia__nome_da_barbearia__icontains=por_barbearia
            )

        if nome_do_barbeiro:
            queryset = queryset.filter(
                barbeiro__username__icontains=nome_do_barbeiro
            )
